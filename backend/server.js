...
app.get(`/platform/:platformID`, async (req, res) => {
  // let {platform} = ;

  console.log(req.params.platformID);

  const response = await fetch(
    `https://api.rawg.io/api/games?key=d3414bb318cb4f30a1f802c153d2afee&page_size=12&platforms=${req.params.platformID}`
  );
  res.json(await response.json());
});
